...
{
#ifdef _DEBUG
    cout << " <<<<<< Called assignment operator for Student \n";
#endif

    if (this != &rhs)                   // Проверка за самоприсвояване
    {
        setName(rhs.name);              // иначе тук ще стане проблем
        setAddress(rhs.address);
        setProgramme(rhs.program);
        setFN(rhs.facNum);
        course = rhs.course;
        setGroup(rhs.group);
        stream = rhs.stream;
    ...
    return *this;                       // винаги връщаме присвоения обект.
}
...
// Проверява валидността на факултетния номер според специалността
bool Student::checkFN(unsigned fn)
{
    assert(program < PROGRAME_COUNT);       // тук невалидна специалност ще означава проблем в класа.

    switch (program)
    {
    case PROGRAME_UNKNOWN:      return true;                // ако не знаем каква е специалността всеки ФН е валиден.
    ...
    default: return false;                                 // ако специалността не е разпозната номера също не е валиден.
    }
    assert(false);
}
